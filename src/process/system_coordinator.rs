...
impl SystemCoordinator {
    ...
    pub fn register_system(&mut self, 
        ...
        // Set frame budget
        self.frame_budget.set_system_budget(system_id, budget_percentage);
        
        // Initialize health monitoring
        {
            let mut health = self.health_monitor.write();
            health.insert(system_id, SystemHealth {
                system_id,
    ...
    fn visit_system(
        ...
        if visiting.contains(&system_id) {
            return Err(EngineError::ValidationFailed(
                format!("Circular dependency detected involving {:?}", system_id)
            ));
        }
        ...
        for &system_id in &self.execution_order {
            ...
            match self.execute_system(system_id) {
                Ok(()) => {
                    ...
                    if history.len() > 60 { // Keep last 60 frames
                        history.pop_front();
                    }
                    
                    // Mark as completed
                    self.current_frame.write().systems_completed.insert(system_id);
                }
                Err(e) => {
    ...
    fn execute_system(&self, system_id: SystemId) -> EngineResult<()> {
        // Mark as in progress
        self.current_frame.write().systems_in_progress.insert(system_id);
        ...
        self.current_frame.write().systems_in_progress.remove(&system_id);
    ...
    fn wait_for_dependencies(&self, system_id: SystemId) -> EngineResult<()> {
        if let Some(deps) = self.dependencies.get(&system_id) {
            ...
            while start.elapsed() < timeout {
                ...
                let all_complete = deps.depends_on.iter()
                ...
                // Check for conflicts
                let has_conflicts = deps.conflicts_with.iter()
            ...
            return Err(EngineError::TimeoutError(
                format!("Timeout waiting for dependencies of {:?}", system_id)
            ));
        }
        ...
        if let Some(strategy) = self.recovery_strategies.get(&system_id) {
            match strategy {
                ...
                RecoveryStrategy::Shutdown => {
                    log::error!("Critical system {:?} failed, initiating shutdown", system_id);
