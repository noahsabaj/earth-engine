...
impl FencePool {
    ...
    pub fn acquire(&self) -> MemoryResult<Arc<Fence>> {
        let mut available = self.available.lock().memory_context("available_fences")?;

        let fence = if let Some(fence) = available.pop_front() {
            fence
        } else {
            // Create new fence
            let mut next_id = self.next_id.lock().memory_context("next_fence_id")?;
            let id = *next_id;
            *next_id += 1;
    ...
    pub fn release(&self, fence: Arc<Fence>) -> MemoryResult<()> {
        let mut active = self.active.lock().memory_context("active_fences")?;
...
impl SyncBarrier {
    ...
    pub fn pending_sync_points(&self) -> MemoryResult<Vec<SyncPoint>> {
        let sync_points = self.sync_points.lock().memory_context("sync_points")?;
...
impl Timeline {
    ...
    pub fn wait_all(&self) -> MemoryResult<()> {
        let stages = self.stages.lock().memory_context("timeline_stages")?;
        if let Some(last_stage) = stages.last() {
            self.sync_barrier
                .wait_for_sync_point(last_stage.sync_point)?;
        }
        Ok(())
    }
}
