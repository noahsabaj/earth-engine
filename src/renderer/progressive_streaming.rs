...
impl ProgressiveStreamer {
    ...
    pub fn new(
        ...
        Self,
        mpsc::UnboundedSender<MeshPacket>,
        mpsc::UnboundedReceiver<MeshUpdate>,
    ) {
    ...
    fn handle_packet(&self, packet: MeshPacket) {
        ...
        let state = states
            .entry(packet.chunk_id)
            .or_insert_with(|| ProgressiveMeshState {
        ...
        let received = state
            .packets_received
            .entry(key.clone())
            .or_insert_with(|| vec![false; packet.total_packets as usize]);

        // Bounds check before accessing array
        let sequence_idx = packet.sequence as usize;
        if sequence_idx >= received.len() {
            eprintln!(
                "Packet sequence {} out of bounds for total packets {}",
                packet.sequence, packet.total_packets
            );
            return;
        }
    ...
    fn process_attribute_update(&self, state: &mut ProgressiveMeshState, packet: &MeshPacket) {
        ...
        for (vertex_idx, new_attrs) in updates {
            if let Some(vertex) = state.base_vertices.get_mut(vertex_idx) {
                ...
                eprintln!(
                    "Vertex index {} out of bounds for base vertices len {}",
                    vertex_idx,
                    state.base_vertices.len()
                );
            }
        }
    }
