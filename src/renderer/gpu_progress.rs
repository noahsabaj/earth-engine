...
impl GpuInitProgress {
        ...
        if let Some(step) = steps.iter_mut().find(|s| s.name == name) {
            ...
            *current += 1;
            ...
            if let Some(details) = details {
                log::info!("[GPU Init Progress] Step {}/{}: {} - Completed in {:.2}s ({})", 
                          *current, steps.len(), name, elapsed, details);
            } else {
                log::info!("[GPU Init Progress] Step {}/{}: {} - Completed in {:.2}s", 
    ...
    pub fn print_summary(&self) -> Result<(), EngineError> {
        ...
        if failed > 0 {
            log::error!("\nFailed steps:");
            for step in steps.iter().filter(|s| matches!(s.status, StepStatus::Failed(_))) {
        ...
        if warnings > 0 {
            log::warn!("\nWarnings:");
            for step in steps.iter().filter(|s| matches!(s.status, StepStatus::Warning(_))) {
...
impl AsyncProgressReporter {
    ...
    pub fn report_progress(&mut self, current: usize, total: usize) {
        ...
        if now.duration_since(self.last_report) >= self.report_interval {
            ...
            log::info!("[{}] Progress: {:.1}% ({}/{}) - Elapsed: {:.1}s", 
                      self.name, progress, current, total, elapsed.as_secs_f32());
    ...
    pub fn finish(self, total: usize) {
        ...
        log::info!("[{}] Completed {} items in {:.2}s", 
                  self.name, total, elapsed.as_secs_f32());
    }
}
