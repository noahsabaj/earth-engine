use crate::world::storage::GpuChunkManager;
use std::sync::Arc;
...
impl ComputePipelineManager {
    pub fn new(device: Arc<wgpu::Device>, queue: Arc<wgpu::Queue>) -> Self {
        // Load compute shader using unified GPU system
        let shader_source = include_str!("../shaders/rendering/chunk_compute.wgsl");
        let validated_shader = match crate::gpu::automation::create_gpu_shader(
            ...
            Err(e) => {
                log::error!(
                    "[ComputePipelineManager] Failed to create chunk compute shader: {}",
                    e
                );
                panic!("Cannot proceed without chunk compute shader");
            }
        };
    ...
    pub fn create_staging_buffers(
        &self,
        output: &mut MeshGenerationOutput,
        max_vertices: u32,
        max_indices: u32,
    ) {
    ...
    pub fn copy_to_staging(
        ...
        if let Some(vertex_staging) = &output.vertex_staging {
            ...
            encoder.copy_buffer_to_buffer(&output.vertex_buffer, 0, vertex_staging, 0, size);
        }

        if let Some(index_staging) = &output.index_staging {
            let size = index_staging.size();
            encoder.copy_buffer_to_buffer(&output.index_buffer, 0, index_staging, 0, size);
        }
    }
}
...
impl GpuMeshGenerator {
    ...
    pub fn generate_mesh(
        ...
        let blocks_per_chunk = core::VOXELS_PER_CHUNK as usize;
        let max_vertices = blocks_per_chunk * 6 * 4;
        let max_indices = blocks_per_chunk * 6 * 6;

        let output = self
            .compute_manager
            .create_mesh_output(max_vertices as u32, max_indices as u32);
