...
impl UnifiedMemoryManager {
    ...
    pub fn get_buffer_binding_params(
        &self,
        offset: u64,
        size: u64,
    ) -> (Arc<wgpu::Buffer>, u64, Option<wgpu::BufferSize>) {
        (
            self.unified_buffer.clone(),
            offset,
    ...
    pub fn get_system_buffer_regions(&self, system: SystemType) -> Vec<(u32, u64, u64)> {
        match system {
            SystemType::TerrainGeneration => vec![
                (
                    0,
                    self.layout.voxel_data_offset,
                    self.layout.voxel_data_size,
                ),
                (
                    1,
                    self.layout.chunk_metadata_offset,
                    self.layout.chunk_metadata_size,
                ),
            ],
            ...
            SystemType::Lighting => vec![
                ...
                (
                    1,
                    self.layout.lighting_data_offset,
                    self.layout.lighting_data_size,
                ),
            ],
            ...
            SystemType::Physics => vec![
                ...
                (
                    1,
                    self.layout.entity_data_offset,
                    self.layout.entity_data_size,
                ),
            ],
            SystemType::Particles => vec![(
                0,
                self.layout.particle_data_offset,
                self.layout.particle_data_size,
            )],
        }
    }
...
impl MemoryStats {
    pub fn print_summary(&self) {
        ...
        println!(
            "  Voxel Data: {:.2} GB",
            self.voxel_data as f64 / (1024.0 * 1024.0 * 1024.0)
        );
        println!(
            "  Chunk Metadata: {:.2} MB",
            self.chunk_metadata as f64 / (1024.0 * 1024.0)
        );
        println!(
            "  Lighting Data: {:.2} GB",
            self.lighting_data as f64 / (1024.0 * 1024.0 * 1024.0)
        );
        println!(
            "  Entity Data: {:.2} MB",
            self.entity_data as f64 / (1024.0 * 1024.0)
        );
        println!(
            "  Particle Data: {:.2} MB",
            self.particle_data as f64 / (1024.0 * 1024.0)
        );
    }
}
