...
impl ThreadPoolManager {
    ...
    fn new(config: ThreadPoolConfig) -> Result<Self, String> {
        ...
        let mut shared_builder = ThreadPoolBuilder::new().num_threads(config.total_threads / 2);
    ...
    pub fn get_pool(&self, category: PoolCategory) -> Arc<ThreadPool> {
        ...
        if pools.len() >= self.config.max_pool_count {
            log::warn!(
                "Maximum pool count ({}) reached, using shared pool for {:?}",
                self.config.max_pool_count,
                category
            );
        ...
        let mut builder = ThreadPoolBuilder::new().num_threads(thread_count);
        ...
        let pool = match builder.build() {
            ...
            Err(_) => {
                log::warn!(
                    "Failed to create pool for {:?}, using shared pool",
                    category
                );
                return self.shared_pool.clone();
            }
        ...
        for (&category, counters) in &self.pool_counters {
            ...
            if completed > 0 {
                ...
                stats
                    ...
                    .insert(category, total_time_ns as f64 / 1_000_000.0);
            }
...
impl ThreadPoolManager {
    ...
    pub fn resize_pool(
        ...
        let mut builder = ThreadPoolBuilder::new().num_threads(new_thread_count);
...
mod tests {
    ...
    fn test_thread_pool_manager_initialization() {
        ...
        let manager =
            ThreadPoolManager::new(config).expect("Failed to create ThreadPoolManager for test");
    ...
    fn test_task_execution() {
        ...
        let manager = Arc::new(
            ThreadPoolManager::new(config)
                .expect("Failed to create ThreadPoolManager for task execution test"),
        );

        let result = manager.execute(PoolCategory::Compute, || 1 + 1);
