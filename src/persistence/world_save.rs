...
impl WorldSave {
    ...
    pub fn load<P: AsRef<Path>>(save_dir: P) -> PersistenceResult<Self> {
        ...
        if !save_dir.exists() {
            return Err(PersistenceError::IoError(
                std::io::Error::new(std::io::ErrorKind::NotFound, "Save directory not found")
            ));
        }
    ...
    pub fn save_world(&mut self, world: &World, metadata: &WorldMetadata) -> Result<(), WorldSaveError> {
        // Save metadata
    ...
    pub fn load_metadata(&self) -> Result<WorldMetadata, WorldSaveError> {
        ...
        let data = fs::read(metadata_path)
            .map_err(|e| WorldSaveError::MetadataLoad(e.into()))?;
        
        let metadata: WorldMetadata = bincode::deserialize(&data)
            .map_err(|e| WorldSaveError::MetadataLoad(e.into()))?;
