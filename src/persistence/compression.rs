...
impl Compressor {
    ...
    fn compress_gzip(&self, data: &[u8]) -> PersistenceResult<Vec<u8>> {
        ...
        encoder.write_all(data).map_err(|e| {
            PersistenceError::CompressionError(format!("Gzip compression failed: {}", e))
        })?;
        encoder.finish().map_err(|e| {
            PersistenceError::CompressionError(format!("Gzip finalization failed: {}", e))
        })
    }
    ...
    fn decompress_gzip(&self, data: &[u8]) -> PersistenceResult<Vec<u8>> {
        ...
        decoder.read_to_end(&mut decompressed).map_err(|e| {
            PersistenceError::CompressionError(format!("Gzip decompression failed: {}", e))
        })?;
        Ok(decompressed)
    }
    ...
    fn compress_zlib(&self, data: &[u8]) -> PersistenceResult<Vec<u8>> {
        ...
        encoder.write_all(data).map_err(|e| {
            PersistenceError::CompressionError(format!("Zlib compression failed: {}", e))
        })?;
        encoder.finish().map_err(|e| {
            PersistenceError::CompressionError(format!("Zlib finalization failed: {}", e))
        })
    }
    ...
    fn decompress_zlib(&self, data: &[u8]) -> PersistenceResult<Vec<u8>> {
        ...
        let mut decompressed = Vec::new();
        decoder.read_to_end(&mut decompressed).map_err(|e| {
            PersistenceError::CompressionError(format!("Zlib decompression failed: {}", e))
        })?;
        Ok(decompressed)
    }
    ...
    fn compress_zstd(&self, data: &[u8]) -> PersistenceResult<Vec<u8>> {
        zstd::encode_all(data, self.compression_level.to_zstd()).map_err(|e| {
            PersistenceError::CompressionError(format!("Zstd compression failed: {}", e))
        })
    }
    ...
    fn decompress_zstd(&self, data: &[u8]) -> PersistenceResult<Vec<u8>> {
        zstd::decode_all(data).map_err(|e| {
            PersistenceError::CompressionError(format!("Zstd decompression failed: {}", e))
        })
    }
    ...
    fn decompress_lz4(&self, data: &[u8]) -> PersistenceResult<Vec<u8>> {
        lz4_flex::decompress_size_prepended(data).map_err(|e| {
            PersistenceError::CompressionError(format!("LZ4 decompression failed: {}", e))
        })
    }
...
mod tests {
    ...
    fn test_compressed_data() {
        ...
        let compressed =
            CompressedData::compress(data, &compressor).expect("Compression should succeed");
