...
use crate::persistence::{
    PersistenceError, PersistenceResult, SaveVersion, WorldMetadata, WorldSave,
};
...
impl MigrationManager {
    ...
    pub fn migrate_world(
        ...
        let mut metadata = world_save.load_metadata().map_err(|e| {
            PersistenceError::MigrationError(format!("Failed to load metadata: {}", e))
        })?;

        // Find migration path
        let path = self
            .find_migration_path(metadata.version, target_version)
            .ok_or_else(|| {
        ...
        for migration in path {
            ...
            println!(
                "Migration step completed: now at version {}",
                metadata.version
            );
        }
...
mod tests {
    ...
    fn test_migration_path() {
        ...
        let path = manager
            .find_migration_path(SaveVersion::new(1, 0, 0), SaveVersion::new(1, 2, 0))
