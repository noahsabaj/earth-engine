...
pub enum SaveOperation {
    /// Save a single chunk
    Chunk { pos: ChunkPos, priority: SavePriority },
    /// Save multiple chunks as batch
    ChunkBatch { positions: Vec<ChunkPos>, priority: SavePriority },
    /// Save player data
    Player { uuid: String, priority: SavePriority },
    /// Save world metadata
...
impl AtomicSaveManager {
    ...
    pub fn queue_operation(&self, operation: SaveOperation) -> PersistenceResult<()> {
        let mut queue = self.operation_queue.lock().persistence_lock("operation_queue")?;
    ...
    pub fn process_next_operation(&self, world: &World) -> PersistenceResult<Option<SaveOperationResult>> {
        let operation = {
            let mut queue = self.operation_queue.lock().persistence_lock("operation_queue")?;
            queue.pop_front()
        };
        ...
        let (success, bytes_written) = match &result {
            Ok(bytes) => {
                if let Ok(mut stats) = self.stats.lock() {
                    ...
                    if total_ops > 0 {
                        let total_time = stats.average_operation_time.as_millis() as u64 * (total_ops - 1) + duration.as_millis() as u64;
                        stats.average_operation_time = Duration::from_millis(total_time / total_ops);
                    }
    ...
    fn execute_operation(&self, operation: &SaveOperation, world: &World) -> PersistenceResult<usize> {
        match operation {
            SaveOperation::Chunk { pos, .. } => {
                self.save_chunk_atomic(world, *pos)
            }
            SaveOperation::ChunkBatch { positions, .. } => {
                self.save_chunk_batch_atomic(world, positions)
            }
            SaveOperation::Player { uuid, .. } => {
                self.save_player_atomic(uuid)
            }
            SaveOperation::Metadata { .. } => {
                self.save_metadata_atomic(world)
            }
            SaveOperation::FullWorld { .. } => {
                self.save_world_atomic(world)
            }
        }
    }
    ...
    fn save_chunk_batch_atomic(&self, world: &World, positions: &[ChunkPos]) -> PersistenceResult<usize> {
        let mut total_bytes = 0;
        let mut locks = Vec::new();
        
        // Acquire all locks first to prevent deadlocks (ordered by path)
        let mut paths: Vec<_> = positions.iter()
    ...
    fn acquire_file_lock(&self, path: &Path) -> PersistenceResult<Arc<Mutex<()>>> {
        ...
        {
            let locks = self.file_locks.read().map_err(|_| PersistenceError::LockPoisoned("file_locks".to_string()))?;
        ...
        {
            let mut locks = self.file_locks.write().map_err(|_| PersistenceError::LockPoisoned("file_locks".to_string()))?;
    ...
    fn validate_chunk_checksum(&self, path: &Path, expected_data: &[u8]) -> PersistenceResult<()> {
        ...
        if actual_data != expected_data {
            return Err(PersistenceError::CorruptedData(
                format!("Checksum validation failed for {}", path.display())
            ));
        }
        Ok(())
    }
...
mod tests {
    ...
    fn test_file_lock_acquisition() {
        ...
        let lock1 = manager.acquire_file_lock(&path).expect("Failed to acquire first lock");
        let lock2 = manager.acquire_file_lock(&path).expect("Failed to acquire second lock");
