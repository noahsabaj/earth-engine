use super::{physics_tables, EntityId, PhysicsData};
use rayon::prelude::*;

// Import physics constants from main physics module
use crate::physics::{
    BLOCK_HALF_EXTENTS, FIXED_TIMESTEP, GRAVITY, PLAYER_HALF_EXTENTS, TERMINAL_VELOCITY,
};
...
impl PhysicsIntegrator {
    ...
    pub fn teleport(physics_data: &mut PhysicsData, entity: EntityId, position: [f32; 3]) {
        ...
        if idx < physics_data.entity_count() {
            ...
            if let Some(bbox) = physics_data.bounding_boxes.get_mut(idx) {
                *bbox =
                    physics_tables::AABB::from_center_half_extents(position, BLOCK_HALF_EXTENTS);
            }
        }
    }
    ...
    pub fn integrate_with_world<W: WorldInterface>(
        &mut self,
    ...
    fn physics_step_with_collision<W: WorldInterface>(
        &mut self,
        physics_data: &mut PhysicsData,
        world: &W,
        dt: f32,
        ...
        for i in 0..count {
            if let (Some(pos), Some(vel), Some(flag), Some(bbox)) = (
                ...
                if flag.is_active() && flag.is_dynamic() {
                    ...
                    let (new_pos, new_vel, collision_flags) =
                        self.resolve_world_collision(world, *pos, delta, *vel, PLAYER_HALF_EXTENTS);
                    ...
                    *bbox = physics_tables::AABB::from_center_half_extents(
                    ...
                    if let Some(physics_flag) = physics_data.flags.get_mut(i) {
                        physics_flag.set_flag(
                            physics_tables::PhysicsFlags::SLEEPING,
                            collision_flags.grounded,
                        );
                    }
                }
            }
    ...
    fn aabb_intersects_block(
        ...
        let block_max = [
            block_pos[0] as f32 + 1.0,
            block_pos[1] as f32 + 1.0,
            block_pos[2] as f32 + 1.0,
        ];

        // AABB intersection test
        entity_min[0] < block_max[0]
            && entity_max[0] > block_min[0]
            && entity_min[1] < block_max[1]
            && entity_max[1] > block_min[1]
            && entity_min[2] < block_max[2]
            && entity_max[2] > block_min[2]
    }
}
