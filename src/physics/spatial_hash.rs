use super::{physics_tables::AABB, EntityId};
...
impl SpatialHash {
    pub fn new(config: SpatialHashConfig) -> Self {
        let expected_cells = ((config.world_max[0] - config.world_min[0]) / config.cell_size)
            as usize
            ...
            for new_cell in &new_cells {
                if !old_cells.contains(new_cell) {
                    cells
                        .entry(*new_cell)
                        .or_insert_with(|| {
                            Vec::with_capacity(self.config.expected_entities_per_cell)
                        })
                        .push(entity);
                }
            }
    ...
    pub fn get_all_potential_pairs(&self) -> Vec<(EntityId, EntityId)> {
        ...
        // Remove duplicates
        let mut unique_pairs: Vec<_> = cell_pairs
            .into_iter()
