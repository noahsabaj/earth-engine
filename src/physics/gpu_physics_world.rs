use crate::{
    gpu::{GpuErrorRecovery, SafeCommandEncoder},
    memory::MemoryManager,
    physics::{physics_tables::PhysicsFlags, EntityId, PhysicsData, FIXED_TIMESTEP},
    world::interfaces::WorldInterface,
    world::{
        compute::hierarchical_physics::{
...
impl GpuPhysicsWorld {
    pub fn new(
        ...
        let entity_buffer_size =
            (max_entities * std::mem::size_of::<PhysicsBodyData>() as u32) as u64;
        ...
        let validated_shader =
            crate::gpu::automation::create_gpu_shader(&device, "gpu_physics", shader_source)?;
    ...
    pub fn add_entity(
        ...
        log::debug!(
            "[GpuPhysicsWorld] Added entity {} at position {:?}",
            entity_id,
            position
        );
        entity_id
    ...
    fn gpu_physics_step<W: WorldInterface>(&mut self, _world: &W) {
        ...
        // Dispatch physics compute shader with error recovery
        let mut encoder =
            self.error_recovery
...

        {
            let encoder_ref = match encoder.encoder() {
                Ok(enc) => enc,
                Err(e) => {
                    log::error!("[GpuPhysicsWorld] Failed to get encoder: {:?}", e);
                    return;
                }
            };

            let mut compute_pass = encoder_ref.begin_compute_pass(&wgpu::ComputePassDescriptor {
        ...
        let commands = match encoder.finish() {
            Ok(cmd) => cmd,
            Err(e) => {
                log::error!("[GpuPhysicsWorld] Failed to finish encoder: {:?}", e);
                return;
            }
        };

        if let Err(e) = self.error_recovery.submit_with_recovery(vec![commands]) {
            log::error!(
                "[GpuPhysicsWorld] Failed to submit physics commands: {:?}",
                e
            );
        }
    }
    ...
    fn upload_entity_data(&self) {
        if self.active_count > 0 {
            ...
            self.queue
                .write_buffer(&self.entity_buffer, 0, bytemuck::cast_slice(data_slice));
        }
    }
    ...
    fn download_entity_data(&mut self) {
        ...
        log::trace!(
            "[GpuPhysicsWorld] GPU physics step complete for {} entities",
            self.active_count
        );
    }
}
