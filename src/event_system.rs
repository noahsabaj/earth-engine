...
impl EventBus {
    ...
    pub fn subscribe<T: Event + 'static, H: EventHandler<T> + 'static>(
        ...
        // Store handler in registry
        {
            ...
            registry.insert(
                format!("{}:{}", subscriber_name, subscription_id),
            ...
            if guaranteed {
                ...
                while queue.len() >= self.config.max_queue_size {
                    if let Some(Reverse(dropped)) = queue.pop() {
                        log::warn!(
                            "Dropping event {} to make space for guaranteed event",
                            dropped.envelope.id
                        );
                        let mut stats = self.stats.write();
    ...
    fn handle_failed_event(&self, mut envelope: EventEnvelope) {
        ...
        if envelope.attempts < envelope.max_attempts {
            ...
            log::error!(
                "Event {} exceeded max retry attempts, dropping",
                envelope.id
            );
            let mut stats = self.stats.write();
    ...
    pub fn replay_events(&self, from_time: Instant, to_time: Instant) -> EngineResult<usize> {
        if !self.config.enable_replay {
            return Err(EngineError::FeatureDisabled(
                "Event replay is disabled".to_string(),
            ));
        }

        let history = self.event_history.read();
        let events_to_replay: Vec<_> = history
            .iter()
...
mod tests {
    ...
    fn test_event_subscription() {
        ...
        let subscription_id =
            bus.subscribe::<TestEvent, _>(handler, None, "test_subscriber".to_string());
    ...
    fn test_event_priority_ordering() {
        ...
        let critical_event = TestEvent {
            message: "critical priority".to_string(),
        };

        // Events should be processed in priority order regardless of publish order
        bus.publish(low_event, None, DeliveryMode::FireAndForget)
            .expect("Failed to publish low event");
        bus.publish(critical_event, None, DeliveryMode::FireAndForget)
            .expect("Failed to publish critical event");
        bus.publish(high_event, None, DeliveryMode::FireAndForget)
            .expect("Failed to publish high event");
