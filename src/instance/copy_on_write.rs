...
use crate::instance::{InstanceId, MetadataKey, MetadataValue};
...
impl<'a> CowBatch<'a> {
    ...
    pub fn create_from_template(mut self, id: InstanceId, template: &'static str) -> Self {
        self.operations
            .push(BatchOp::CreateFromTemplate(id, template));
        self
    }
...
mod tests {
    ...
    fn test_template_system() {
        ...
        cow.create_from_template(id1, "sword")
            .expect("Failed to create instance from sword template");
        cow.create_from_template(id2, "sword")
            .expect("Failed to create second instance from sword template");

        // Both share template
        assert_eq!(cow.get(&id1, "damage"), Some(MetadataValue::I32(10)));
        assert_eq!(cow.get(&id2, "damage"), Some(MetadataValue::I32(10)));

        // Modify one
        cow.set(id1, "damage", MetadataValue::I32(15))
