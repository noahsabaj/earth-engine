...
use crate::instance::{InstanceId, InstanceType, InstanceData, MetadataValue, MetadataStore};
use bit_vec::BitVec;
                ...
                for i in 0..self.data.ids.len() {
                    if matches[i] {
                        ...
                        let in_range = match self.metadata.get(id, key) {
                            ...
                            Some(MetadataValue::I32(v)) => (*min <= v as f64) && ((v as f64) <= *max),
                            Some(MetadataValue::I64(v)) => (*min <= v as f64) && ((v as f64) <= *max),
                            _ => false,
                        };
                        if !in_range {
                            matches.set(i, false);
...
impl QueryIndices {
    ...
    pub fn get_by_creator(&self, creator: &InstanceId) -> &[usize] {
        self.by_creator.get(creator).map(|v| v.as_slice()).unwrap_or(&[])
    }
}
