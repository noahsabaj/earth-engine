...
pub fn update_colors(particles: &mut ParticleData) {
    for i in 0..particles.count {
        ...
        match particles.color_curve_type[i] {
            ...
            2 => { // Linear interpolation (params store end color in rgb channels)
                let start_a = particles.color_a[i];
                particles.color_r[i] = particles.color_r[i] + (particles.color_curve_param1[i] - particles.color_r[i]) * t;
                particles.color_g[i] = particles.color_g[i] + (particles.color_curve_param2[i] - particles.color_g[i]) * t;
                particles.color_b[i] = particles.color_b[i] + (particles.color_curve_param1[i] - particles.color_b[i]) * t;
                particles.color_a[i] = start_a * (1.0 - t);
            },
            3 => { // Temperature
                let temp = particles.color_curve_param1[i] + (particles.color_curve_param2[i] - particles.color_curve_param1[i]) * t;
...
pub fn update_emitters(
    ...
    while i < emitters.count {
        ...
        for _ in 0..to_spawn {
            ...
            let spawn_pos = generate_spawn_position(
                emitters,
                i,
                &mut rng,
            );
            ...
            let velocity = base_vel + Vec3::new(
...
pub fn spawn_particle(
    ...
    particles.emission_intensity.push(properties.emission_intensity);
    
    particles.size_curve_type.push(properties.size_curve_type);
    particles.size_curve_param1.push(properties.size_curve_param1);
    particles.size_curve_param2.push(properties.size_curve_param2);
    particles.size_curve_param3.push(properties.size_curve_param3);
    
    particles.color_curve_type.push(properties.color_curve_type);
    particles.color_curve_param1.push(properties.color_curve_param1);
    particles.color_curve_param2.push(properties.color_curve_param2);
