...
use crate::particles::particle_data::{
    prepare_render_data, EmitterData, ParticleData, ParticleGPUData, MAX_PARTICLES,
};
use crate::particles::update::{
    apply_force_field, apply_turbulence, apply_vortex, spawn_particle, update_emitters,
    update_particles,
};
...
impl DOPParticleSystem {
    ...
    pub fn add_emitter(
        ...
        self.emitters
            .duration
            .push(duration.map_or(-1.0, |d| d.as_secs_f32()));
    ...
    fn update_stats(&mut self, initial_count: usize, spawned: usize) {
        ...
        self.stats.capacity_used =
            self.particles.count as f32 / self.particles.position_x.capacity() as f32;
...
pub fn create_fire_effect(system: &mut DOPParticleSystem, position: Vec3, size: f32) {
    // Main fire emitter
    let fire_id =
        system.add_sphere_emitter(position, size * 0.5, ParticleType::Fire, 100.0 * size, None);
    ...
    // Sparks
    let spark_id = system.add_emitter(position, ParticleType::Spark, 20.0 * size, None);
    system.set_emitter_velocity(spark_id, Vec3::new(0.0, 3.0, 0.0), 1.0);
}

/// Create rain effect
pub fn create_rain_effect(
