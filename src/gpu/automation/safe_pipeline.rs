...
use wgpu::{ComputePipeline, Device, PipelineLayout, RenderPipeline};
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::LayoutMismatch { expected, found } => {
                write!(
                    f,
                    "Pipeline layout mismatch: expected {}, found {}",
                    expected, found
                )
            }
            Self::MissingBinding {
                ...
                write!(
                    f,
                    "Missing required binding: group {}, binding {}, name: {}",
                    group, binding, name
                )
            }
            Self::InvalidVertexAttribute { location, expected } => {
                write!(
                    f,
                    "Invalid vertex attribute: location {}, expected {}",
                    location, expected
                )
            }
...
impl<'a> TypedComputePipelineBuilder<'a> {
    ...
    pub fn build(self) -> PipelineResult<ComputePipeline> {
        ...
        match std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
            ...
            Ok(pipeline) => Ok(pipeline),
            Err(_) => Err(PipelineError::CreationFailed(
                "Compute pipeline creation panicked - check shader/layout compatibility"
                    .to_string(),
            )),
        }
    }
