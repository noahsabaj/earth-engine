...
impl ShaderValidator {
    ...
    fn generate_parse_suggestions(
        &self,
        _error: &naga::front::wgsl::ParseError,
        line: &str,
    ) -> Vec<String> {
        ...
        if line.contains("var") && !line.contains(":") {
            suggestions
                .push("Add type annotation after variable name (e.g., var name: type)".to_string());
        }
        ...
        if error_str.contains("UnknownType") || error_str.contains("unknown type") {
            suggestions
                .push("Check type spelling and ensure all custom types are defined".to_string());
            suggestions.push(
                "Common WGSL types: u32, i32, f32, vec2<f32>, vec3<f32>, vec4<f32>".to_string(),
            );
        }
    ...
    fn generate_validation_suggestions(&self, error: &naga::valid::ValidationError) -> Vec<String> {
        ...
        if error_str.contains("Function") || error_str.contains("function") {
            suggestions.push(
                "Check function signatures for correct parameter and return types".to_string(),
            );
            suggestions.push(
                "Ensure all functions have proper stage annotations (@vertex, @fragment, @compute)"
                    .to_string(),
            );
        }
