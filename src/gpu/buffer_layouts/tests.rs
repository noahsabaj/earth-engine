...
mod tests {
    ...
    fn test_compute_dispatch_params() {
        use compute::{ComputeDispatchParams, workgroup_sizes};
    ...
    fn test_terrain_params_conversion() {
        ...
        for i in 0..aos.distribution_count as usize {
            assert_eq!(aos.distributions[i].block_id, aos2.distributions[i].block_id);
            assert_eq!(aos.distributions[i].min_y, aos2.distributions[i].min_y);
            assert_eq!(aos.distributions[i].threshold, aos2.distributions[i].threshold);
        }
    }
}