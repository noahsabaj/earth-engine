...
impl PlayerInterest {
    ...
    pub fn get_view_distance(&self) -> f32 {
        self.view_distance_override
            .unwrap_or(MAX_ENTITY_VIEW_DISTANCE)
    }
}
...
impl InterestManager {
    ...
    pub fn get_interest_changes(
        &self,
        player_id: u32,
        old_entities: &HashSet<u32>,
    ) -> (Vec<u32>, Vec<u32>) {
    ...
    pub fn get_stats(&self) -> InterestStats {
        ...
        InterestStats {
            ...
            total_subscriptions,
            avg_entities_per_player: if self.players.is_empty() {
                0.0
            } else {
                self.players
                    .values()
                    .map(|i| i.interested_entities.len())
                    .sum::<usize>() as f32
                    / self.players.len() as f32
            },
            avg_chunks_per_player: if self.players.is_empty() {
                0.0
            } else {
                self.players
                    .values()
                    .map(|i| i.interested_chunks.len())
                    .sum::<usize>() as f32
                    / self.players.len() as f32
            },
        }
...
pub fn interest_update_entity_position(
    manager: &mut InterestManager,
    entity_id: u32,
    position: Vec3,
) {
    ...
    manager
        .entity_positions
        .insert(entity_id, (position, new_region));
}
...
pub fn interest_update_player_interests(manager: &mut InterestManager, player_id: u32) {
    let Some(interest) = manager.players.get(&player_id) else {
        return;
    };
...
mod tests {
    ...
    fn test_interest_management() {
        ...
        // Check interests
        let p1_entities = manager
            .get_interested_entities(1)
        ...
        let p3_entities = manager
            .get_interested_entities(3)
