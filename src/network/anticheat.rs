...
impl AntiCheat {
    ...
    pub fn validate_movement(
        ...
        if !result.is_valid {
            self.add_violation(
                player_id,
                ViolationType::Movement(result.violation_type.clone()),
            );
        }
    ...
    pub fn validate_block_interaction(
        ...
        if !self
            .rate_limiter
            .check_action(player_id, ActionType::BlockInteraction)
        {
    ...
    pub fn validate_mining(
        ...
        let valid =
            self.mining_validator
                .validate_mining_time(block_id, tool_effectiveness, time_spent);
    ...
    pub fn validate_combat(&mut self, player_id: u32, action: CombatAction) -> bool {
        // Check rate limit
        ...
        match action {
            CombatAction::Attack {
                ...
                .combat_validator
                .validate_attack(player_pos, player_rotation, target_pos),
...
impl ViolationType {
    fn severity(&self) -> f32 {
        match self {
            ...
            ViolationType::InvalidReach { distance } => 10.0 * (distance - 5.0).max(0.0),
...
impl MovementViolation {
    fn severity(&self) -> f32 {
        match self {
            MovementViolation::TooFast { speed } => 5.0 * (speed - 15.0).max(0.0),
            MovementViolation::TooHigh { height } => 10.0 * (height - 2.0).max(0.0),
            MovementViolation::InvalidFlight => 30.0,
            MovementViolation::Teleport { distance } => 20.0 * (distance / 10.0),
        }
    }
}
...
impl MovementValidator {
    ...
    fn validate(
        ...
        if horizontal_speed > max_speed * 1.1 {
            // 10% tolerance
            return ValidationResult {
                is_valid: false,
        ...
        if delta.y > 0.0 && on_ground {
            ...
            if delta.y > self.max_jump_height {
                return ValidationResult {
                    is_valid: false,
                    corrected_position: None,
                    violation_type: MovementViolation::TooHigh { height: delta.y },
                };
            }
...
impl ActionRateLimiter {
    ...
    fn check_action(&mut self, player_id: u32, action_type: ActionType) -> bool {
        ...
        let player_actions = self
            .player_actions
            .entry(player_id)
            .or_insert_with(HashMap::new);

        let actions = player_actions.entry(action_type).or_insert_with(Vec::new);
...
impl MiningValidator {
    fn new() -> Self {
        ...
        times.insert(BlockId(typed_blocks::DIRT), 0.75);
        times.insert(BlockId(typed_blocks::GRASS), 0.6);
        times.insert(BlockId(typed_blocks::STONE), 2.25);
        times.insert(BlockId(typed_blocks::WOOD), 0.3);
        times.insert(BlockId(typed_blocks::SAND), 0.6);
