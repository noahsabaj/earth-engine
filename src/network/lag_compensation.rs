...
impl LagCompensation {
        ...
        F: FnOnce(&PlayerStateSnapshot, &[PlayerStateSnapshot]) -> Option<R>,
    {
        ...
        let rewind_time = self
            .server_time
            .checked_sub(Duration::from_millis(client_ping_ms as u64 / 2))
    ...
    fn get_state_from_history(
        &self,
        history: &VecDeque<PlayerStateSnapshot>,
        time: Instant,
    ) -> Option<PlayerStateSnapshot> {
        ...
        match (before, after) {
            (Some(before_state), Some(after_state)) => {
                ...
                let total_time = after_state
                    .timestamp
                    .duration_since(before_state.timestamp)
                    .as_secs_f32();
    ...
    pub fn validate_hit(
        ...
        self.validate_action(
            ...
            self.server_time,
            ping_ms,
            |shooter_state, other_players| {
                // Find the target player
                let target_state = other_players.iter().find(|p| p.player_id == target_id)?;
...
pub fn lag_compensation_add_player_snapshot(
    lag_comp: &mut LagCompensation,
    snapshot: PlayerStateSnapshot,
) {
    let history = lag_comp
        .player_history
    ...
    let cutoff_time = lag_comp
        .server_time
        .checked_sub(lag_comp.max_rewind_time)
...
pub fn lag_compensation_add_world_snapshot(
    lag_comp: &mut LagCompensation,
