...
impl VoxelBvh {
    ...
    pub fn build_from_chunks(
        ...
        let mut primitives: Vec<Primitive> = chunk_positions.iter().enumerate()
            .map(|(i, pos)| {
                let min = Point3::new(
        ...
        queue.write_buffer(&self.primitive_buffer, 0, bytemuck::cast_slice(&primitive_indices));
    }
    ...
    fn build_recursive(
        ...
        // Build children
        let left_child = self.build_recursive(nodes, primitive_indices, primitives, start, mid, depth + 1);
        let right_child = self.build_recursive(nodes, primitive_indices, primitives, mid, end, depth + 1);
        ...
        for axis in 0..3 {
            ...
            for i in 1..samples {
                ...
                if left_count > 0 && right_count > 0 {
                    ...
                    let cost = 1.0 + (left_area * left_count as f32 + right_area * right_count as f32) / parent_area;
    ...
    pub fn get_stats(&self) -> BvhStats {
        BvhStats {
            ...
            memory_usage_mb: (self.node_count as f32 * std::mem::size_of::<BvhNode>() as f32 
                + self.primitive_count as f32 * 4.0) / (1024.0 * 1024.0),
        }
    }
}
