...
impl SystemMonitor {
    ...
    pub fn record_metric(&self, system: MonitoredSystem, metric_type: MetricType, value: f64) {
        ...
        let history = metrics.entry(key).or_insert_with(|| MetricHistory::new(300)); // 5 minutes at 60fps
    ...
    pub fn end_profiling(&self, name: &str) -> Option<Duration> {
        ...
        if let Some(session) = active_sessions.remove(name) {
            ...
            let completed = CompletedSession {
                ...
                nested_durations: session.nested_sessions.iter()
            ...
            // Record as metric
            self.record_metric(session.system, MetricType::FrameTime, duration.as_secs_f64() * 1000.0);
    ...
    pub fn record_error(&self, system: MonitoredSystem, error: &EngineError) {
        ...
        // Create alert
        let alert_id = self.next_alert_id.fetch_add(1, std::sync::atomic::Ordering::SeqCst);
    ...
    fn check_thresholds(&self, system: MonitoredSystem, metric_type: MetricType, value: f64) {
        let (warning_threshold, critical_threshold) = match metric_type {
            MetricType::FrameTime => (self.thresholds.frame_time_warning_ms, self.thresholds.frame_time_critical_ms),
            MetricType::MemoryUsage => (self.thresholds.memory_warning_percent, self.thresholds.memory_critical_percent),
            MetricType::CPUUsage => (self.thresholds.cpu_warning_percent, self.thresholds.cpu_critical_percent),
        ...
        let alert = SystemAlert {
            ...
            level: alert_level,
            system,
            message: format!("{:?} {} exceeded threshold", metric_type, 
                match metric_type {
            ...
            threshold: Some(if alert_level == AlertLevel::Critical { critical_threshold } else { warning_threshold }),
        };
    ...
    fn update_system_health(&self, system: MonitoredSystem, metric_type: MetricType, value: f64) {
        ...
        let health_impact = match metric_type {
            MetricType::FrameTime if value > self.thresholds.frame_time_critical_ms => HealthStatus::Critical,
            MetricType::FrameTime if value > self.thresholds.frame_time_warning_ms => HealthStatus::Warning,
            MetricType::ErrorRate if value > self.thresholds.error_rate_critical => HealthStatus::Critical,
            MetricType::ErrorRate if value > self.thresholds.error_rate_warning => HealthStatus::Warning,
            _ => HealthStatus::Good,
        };
    ...
    fn calculate_overall_status(&self, system_statuses: &HashMap<MonitoredSystem, HealthStatus>) -> HealthStatus {
        if system_statuses.values().any(|&status| status == HealthStatus::Critical) {
            HealthStatus::Critical
        } else if system_statuses.values().any(|&status| status == HealthStatus::Warning) {
            HealthStatus::Warning
        } else if system_statuses.values().all(|&status| status == HealthStatus::Excellent) {
    ...
    fn generate_performance_summary(&self) -> PerformanceSummary {
        ...
        let frame_time = metrics.get(&(MonitoredSystem::Renderer, MetricType::FrameTime))
            .map(|h| h.average)
            .unwrap_or(16.67); // Default to 60 FPS
        
        let memory_usage = metrics.get(&(MonitoredSystem::Memory, MetricType::MemoryUsage))
            .map(|h| h.average)
            .unwrap_or(0.0);
        
        let cpu_usage = metrics.get(&(MonitoredSystem::ThreadPool, MetricType::CPUUsage))
            .map(|h| h.average)
            .unwrap_or(0.0);
    ...
    fn calculate_error_rate(&self) -> f64 {
        ...
        let total_errors: usize = error_history.values()
            .map(|errors| {
                errors.iter()
    ...
    fn generate_recommendations(&self, 
        ...
        if performance_summary.average_frame_time_ms > 20.0 {
            recommendations.push("Consider reducing render distance or graphics quality".to_string());
        }
        
        if performance_summary.memory_usage_mb > 2048.0 {
            recommendations.push("High memory usage detected - consider increasing garbage collection frequency".to_string());
        }
        
        if performance_summary.error_rate_per_minute > 1.0 {
            recommendations.push("Elevated error rate - check system logs for recurring issues".to_string());
        }
