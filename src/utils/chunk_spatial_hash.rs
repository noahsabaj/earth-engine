...
pub fn chunk_pos_to_index(pos: ChunkPos) -> Option<usize> {
    // Check bounds
    if pos.x < -MAX_CHUNK_COORD
        || pos.x >= MAX_CHUNK_COORD
        || pos.y < -MAX_CHUNK_COORD
        || pos.y >= MAX_CHUNK_COORD
        || pos.z < -MAX_CHUNK_COORD
        || pos.z >= MAX_CHUNK_COORD
    {
        return None;
    }
...
impl<T> ChunkSpatialHash<T> {
    ...
    pub fn get(&self, pos: ChunkPos) -> Option<&T> {
        chunk_pos_to_index(pos).and_then(|index| self.data[index].as_ref())
    }
    ...
    pub fn iter(&self) -> impl Iterator<Item = (ChunkPos, &T)> {
        self.active_indices.iter().filter_map(move |&index| {
            self.data[index]
                .as_ref()
                .map(|value| (index_to_chunk_pos(index), value))
        })
    }
...
pub fn spatial_hash_insert<T>(
    hash: &mut ChunkSpatialHash<T>,
    pos: ChunkPos,
    value: T,
) -> Option<T> {
...
pub fn spatial_hash_get_mut<T>(hash: &mut ChunkSpatialHash<T>, pos: ChunkPos) -> Option<&mut T> {
    chunk_pos_to_index(pos).and_then(|index| hash.data[index].as_mut())
}
...
pub fn distance_hash_insert<T>(
    hash: &mut ChunkDistanceHash<T>,
    pos: ChunkPos,
    value: T,
) -> Option<T> {
