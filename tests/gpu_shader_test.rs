...
fn test_terrain_generation_shader_compiles() {
    ...
    pollster::block_on(async {
        ...
        use hearth_engine::world_gpu::terrain_generator_soa::TerrainGeneratorSOA;
        use hearth_engine::world_gpu::world_buffer::WorldBuffer;
...
fn test_chunk_metadata_structure() {
    ...
    let chunk_pos = ChunkPos { x: 10, y: 5, z: -15 };
...
fn test_gpu_constants_match() {
    use hearth_engine::gpu::constants;
    
    // Verify constants are correct
    assert_eq!(constants::CHUNK_SIZE, 50);
    assert_eq!(constants::CHUNK_SIZE_F32, 50.0);
    assert_eq!(constants::VOXELS_PER_CHUNK, 125000);
